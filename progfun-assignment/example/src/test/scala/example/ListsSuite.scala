...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers") {
   ...
   test("sum of negative numbers") {
    assert(sum(List(-1,-5,-2,-10)) === -18)
  }
   
   test("sum applyed on empty list") {
    assert(sum(List()) === 0)
  }
   
   test("sum in list with repeated elements") {
    assert(sum(List(1, 2, 4, 1, 2, 4)) === 14)
  }
  ...
  test("max throws an exception if its argument is an empty list") {
   ...
   test("max in list with repeated elements") {
    assert(max(List(1, 2, 4, 1, 2, 4)) === 4)
  }
